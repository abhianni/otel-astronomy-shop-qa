...
automation/
  src/test/java/
    commerce/          # 2 QAs — cart, checkout, payment contracts
    catalog/           # 1 QA — product-catalog contracts + browse HTTP smoke
    platform/          # 1 QA — health checks, flagd fixtures, flake quarantine helpers, shipping/currency/post-order (Kafka + Postgres) contracts
...
|---|---|---|
| Commerce QA | P0 core buy-path automation (checkout/cart/payment), nightly reds on checkout | Every unit test inside Go/Java/.NET services |
| Catalog QA | Product list/get contracts, currency display smoke (browse-level only) | Payment decline matrix, currency conversion correctness |
| Platform QA | CI workflow, artifact upload, env readiness job, quarantine process, shipping/currency service contracts, post-order flow (ship → empty cart → email → Kafka) — same owner as the Postgres/Kafka paths these terminate in | Feature assertions for commerce |
